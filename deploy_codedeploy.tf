resource "local_file" "appspec" {
  ...
  content = templatefile("${path.module}/files/codedeploy/appspec.yml.tpl", {
...
resource "null_resource" "deploy_codedeploy" {
  ...
  provisioner "local-exec" {
    command = "aws deploy create-deployment --cli-input-yaml file://${path.module}/${aws_codedeploy_app.main[count.index].name}.yml"
    environment = {
